...
const cObsState = {
  sessions: {},    // { 'sess_c1': ['read_docs', ...], ... }
};

const cGovState = {
  violations: [], // [{ session, tool, decision }, ...]
};

// ── Animation queues (one per row per scenario) ───────────────────────────────
...
function renderAnnotatedTrace(sc, trace) {
  ...
  const fields = [
    ...
    {
      key: 'checks.injection_scan.found',
      note: 'Pre-execution scan of request params for shell commands, crontab paths',
      val: trace.checks?.injection_scan?.found,
    },
    {
      key: 'checks.response_injection_scan.found',
      note: 'Post-execution scan of target response for hidden injection payloads (HTML comment directives)',
      val: trace.checks?.response_injection_scan?.found,
      evidence: trace.checks?.response_injection_scan?.found
        ? trace.checks?.response_injection_scan?.evidence
        : null,
    },
  ...
  el.innerHTML = fields.map(f => {
    ...
    const valContent = isNull
      ? '<span class="ann-null">—</span>'
      : esc(String(v));
    const evidBlock = f.evidence
      ? `<div class="ann-evidence">${esc(f.evidence)}</div>`
      : '';
...
// ── PII accumulation tracker (Scenario A govern) ──────────────────────────────
...
function renderPIITracker() {
  ...
  const rows = activeSess.map(sid => {
    ...
    const cumBefore = running;
    running += count;
    const cumAfter = running;

    let statusHtml = '';
    if (cumAfter >= BLOCK_T)  statusHtml = `<span class="pii-status block">BLOCKED</span>`;
    else if (cumAfter >= FLAG_T) statusHtml = `<span class="pii-status flag">FLAGGED</span>`;

    const bar = '█'.repeat(Math.min(count, 8));
    return `<div class="pii-row">
      <span class="pii-sess">${sid}</span>
      <span class="pii-bar">${bar}</span>
      <span class="pii-count">${count} fields</span>
      <span class="pii-cumul">total: ${cumAfter}</span>
      ${statusHtml}
    </div>`;
  }).join('');
  ...
  const statusText  = piiState.blocked
    ? `BLOCKED — cross-session threshold exceeded (${piiState.cumulative}/${BLOCK_T})`
    : piiState.flagged
    ? `FLAGGED — cumulative reads at ${piiState.cumulative}/${FLAG_T} flag threshold`
...
function resetUI() {
  ...
  // Reset policy tracker to initial static HTML
